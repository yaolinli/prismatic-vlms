...
do
    ...
    do
        ...
        --model.vision_backbone_id $vision_backbone \
        --model.image_resize_strategy $resize_strategy \
        --model.llm_backbone_id $model_name  --run_id "one-stage_"${model_name}"+"${vision_backbone}"+"${projector} --model.arch_specifier 'no-align+'$projector --model.finetune_per_device_batch_size 2
    done
done
...
CUDA_VISIBLE_DEVICES=$gpu_id torchrun --standalone --nnodes 1 --nproc-per-node $gpu_num scripts/pretrain.py \
  ...
  --model.vision_backbone_id $vision_backbone \
  --model.image_resize_strategy $resize_strategy \
  --model.llm_backbone_id $model_name  --run_id "one-stage_"${model_name}"+"${vision_backbone}"+"${projector} --model.arch_specifier 'no-align+'$projector --model.finetune_per_device_batch_size 1
